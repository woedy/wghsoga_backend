...
def add_year_group_bank_account(request):
    payload = {}
    data = {}
    errors = {}
    ...
    if request.method == 'POST':
        ...
        try:
            bank_account = BankAccount.objects.get(year_group=year_group)
            errors['year_group'] = ['Account for this year group already exist.']
        except:
            pass
...
@permission_classes([IsAuthenticated, ])
@authentication_classes([TokenAuthentication, ])
def get_all_bank_account_view(request):
    payload = {}
    data = {}
    errors = {}
    ...
    accounts = BankAccount.objects.all().filter(is_archived=False ).order_by('-created_at')
    ...
    try:
        paginated_accounts = paginator.page(page_number)
    except PageNotAnInteger:
        paginated_accounts = paginator.page(1)
    except EmptyPage:
        paginated_accounts = paginator.page(paginator.num_pages)

    all_accounts_serializer = BankAccountSerializer(paginated_accounts, many=True)


    data['accounts'] = all_accounts_serializer.data
    data['pagination'] = {
        'page_number': paginated_accounts.number,
        'total_pages': paginator.num_pages,
        'next': paginated_accounts.next_page_number() if paginated_accounts.has_next() else None,
        'previous': paginated_accounts.previous_page_number() if paginated_accounts.has_previous() else None,
    }
...
def delete_bank_account(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            errors['account_id'] = ['Account does not exist.']
