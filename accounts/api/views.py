...
class AdminLogin(APIView):
    ...
    def post(self, request):
        ...
        if qs.admin == False:
            errors['email'] = ['User not an admin.']
...
def list_all_users_view(request):
    ...
    users = User.objects.filter(is_archived=False, admin=False, verified=True).order_by('-timestamp')
...
@permission_classes([IsAuthenticated])
@authentication_classes([TokenAuthentication])
def admin_list_all_users_view(request):
    payload = {}
    data = {}
    errors = {}
    ...
    users = User.objects.filter(is_archived=False, admin=False).order_by('-timestamp')
...
@permission_classes([IsAuthenticated, ])
@authentication_classes([TokenAuthentication, ])
def admin_get_user_details_view(request):
    payload = {}
    data = {}
    errors = {}
